...
def main():
    ...
    while count < 1000:
        try:
            tweets = api.search(q='hillary clinton OR donald trump OR marco rubio OR bernie sanders', rpp=100, since_id=last_id)
            tweetJson = dict()
            if len(tweets) > 0:
                with open('testC/c' + str(count) + '.json', 'w') as jsonfile:
